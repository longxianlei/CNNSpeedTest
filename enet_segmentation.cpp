/* Add ENet segmentation code here.*/
...
"{model m   |./weights/model-cityscapes.net| path to Torch .net model file (model_best.net) }"
"{image i   |./image/test_img2.bmp| path to image file }"
"{result r  |./result/| path to save output blob (optional, binary format, NCHW order) }"
...
int main(int argc, char** argv)
{
	...
	resize(img, img, Size(800, 600), 0, 0, 1);
	...
	Mat inputBlob = blobFromImage(img, 1. / 255, Size(512, 512), Scalar(), true, false);   //Convert Mat to batch of images
	//! [Prepare blob]
	...
	std::cout << "Inference time, ms: " << tm.getTimeMilli() / 100 << std::endl;
